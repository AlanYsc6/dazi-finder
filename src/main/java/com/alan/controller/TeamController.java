...
public class TeamController {
     ...
     */
    @GetMapping("/list")
    public BaseResponse<List<TeamUserVO>> listByCreator(TeamDTO teamDTO, HttpServletRequest request) {
        User loginUser = userService.getLoginUser(request);
        log.info("teamDTO: {}, loginUser: {}", teamDTO, loginUser);
        if (teamDTO == null) {
            log.info("teamDTO is null");
        ...
        List<TeamUserVO> list = teamService.listTeams(teamDTO, true);
        if (list == null) {
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "查询队伍列表为空");
        }
        return ResultUtils.success(list);
     ...
     */
    @GetMapping("/list")
    public BaseResponse<List<TeamUserVO>> listByJoiner(TeamDTO teamDTO, HttpServletRequest request) {
        User loginUser = userService.getLoginUser(request);
        log.info("teamDTO: {}, loginUser: {}", teamDTO, loginUser);
        if (teamDTO == null) {
            log.info("teamDTO is null");
        ...
        List<Long> teamIdList=userTeamService.getIdListByUserId(loginUser.getId());
